...
import { EngineType, FeedPost, PostStatus } from '../types';
import { VeoLogo } from './icons';
import { AlertCircle, Download, Sparkles, Zap } from 'lucide-react';
...
const VideoCard: React.FC<{ post: FeedPost }> = ({ post }) => {
  ...
  const handleDownload = async (e: React.MouseEvent) => {
    ...
    try {
        if (post.videoUrl.startsWith('blob:')) {
            ...
            // Determine extension
            const isImage = post.videoUrl.includes('.png') || post.videoUrl.includes('.jpg') || post.videoUrl.includes('.jpeg');
            a.download = `cameo-${post.id}.${isImage ? 'png' : 'mp4'}`;
  ...
  const renderContent = () => {
    const isImage = post.videoUrl && (post.videoUrl.includes('.png') || post.videoUrl.includes('.jpg') || post.videoUrl.includes('.jpeg') || post.videoUrl.includes('.webp'));
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-gray-900 p-6 text-center">
            ...
            <div className="relative z-10 flex flex-col items-center gap-4">
              <div className="relative">
                ...
                {post.engineType === EngineType.COMFY_UI ? (
              ...
              <div>
                <p className="text-sm font-bold uppercase tracking-widest text-white mb-1 font-bogle animate-pulse">
                    {post.engineType === EngineType.COMFY_UI ? 'Processing Workflow' : 'Generating Scene'}
                </p>
      ...
      case PostStatus.ERROR:
        return (
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-gray-900/80 border border-red-500/30 p-6 text-center">
            ...
            <p className="text-xs text-red-300 line-clamp-3 px-2 break-words">{post.errorMessage || "An unexpected error occurred."}</p>
          </div>
        );
      ...
      <div className={`absolute bottom-0 left-0 w-full p-5 flex items-end justify-between z-20 pt-16 bg-gradient-to-t from-black via-black/50 to-transparent transition-opacity duration-300 ${status !== PostStatus.SUCCESS ? 'opacity-50 pointer-events-none' : ''}`}>
        <div className="flex-1 mr-4 pointer-events-none">
          <div className="flex items-center gap-2.5 mb-2">
            ...
            {post.templateLabel && (
                <span className="text-[10px] bg-white/10 px-1.5 py-0.5 rounded border border-white/10 uppercase tracking-wide text-white/60">
                    {post.templateLabel}
                </span>
            )}
          </div>
